...
bool itas109::CLog4CPP::LogOutHex(unsigned char* text, int length, bool isOverlayWrite)
{
	...
	if (fp)
	{
		...
		if (i == length - 1)
		{
			_ftprintf_s(fp, _T("%02X\n"), text[i]);
		}
		else
		{
			_ftprintf_s(fp, _T("%02X "), text[i]);
		}
#else
